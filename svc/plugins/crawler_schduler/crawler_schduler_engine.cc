...
bool SchdulerEngineImpl::SetCrawlerSchduler(
    const int32 id, base_logic::CrawlerScheduler* schduler) {
...
bool SchdulerEngineImpl::GetCrawlerSchduler(
    const int32 id, base_logic::CrawlerScheduler* schduler) {
  CrawlerSchdulerManager* schduler_mgr =
      CrawlerSchdulerEngine::GetCrawlerSchdulerManager();
...
bool SchdulerEngineImpl::FindCrawlerSchduler(
    const int socket, base_logic::CrawlerScheduler* schduler) {
  CrawlerSchdulerManager* schduler_mgr =
      CrawlerSchdulerEngine::GetCrawlerSchdulerManager();
...
bool CrawlerSchdulerManager::SetCrawlerSchduler(
    const int32 id, base_logic::CrawlerScheduler* schduler) {
...
bool CrawlerSchdulerManager::FindCrawlerSchduler(
    const int socket, base_logic::CrawlerScheduler* schduler) {
  base_logic::RLockGd lk(lock_);
...
bool CrawlerSchdulerManager::CheckHeartPacket(const int socket) {
  ...
  if (0 != socket) {
    ...
    base_logic::CrawlerScheduler& crawler_schduler = schduler_cache_
        ->socket_schduler_map_[socket];
    base_logic::CrawlerScheduler& crawler_schduler_from_schduler_map =
        schduler_cache_->crawler_schduler_map_[crawler_schduler.id()];
  ...
  SCHDULER_MAP::iterator it = schduler_cache_->crawler_schduler_map_.begin();
