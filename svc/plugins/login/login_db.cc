...
bool LoginDB::GetCookie(std::list<base_logic::LoginCookie>* cookie_list, const int64 id,
...
void LoginDB::CallBackGetCookie(void* param,
...
void LoginDB::CallBackGetCookies(void* param,
        base_logic::Value* value) {
    base_logic::DictionaryValue* dict =
        (base_logic::DictionaryValue*)(value);
    base_logic::ListValue* list = new base_logic::ListValue();
    ...
    int32 num = engine->RecordCount();
    int64 attr_id;
    LoginSchdulerManager* schduler_manager = LoginSchdulerEngine::GetLoginSchdulerManager();
    if (num > 0) {
        while ( rows = (*(MYSQL_ROW*)(engine->FetchRows())->proc) ) {
            base_logic::DictionaryValue* info_value =
                    new base_logic::DictionaryValue();
