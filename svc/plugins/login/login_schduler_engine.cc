...
void LoginSchdulerManager::CheckBatchCookie(const int64 plat_id) {
    ...
    bool r = false;
    r = base::MapGet<COOKIE_MAP, COOKIE_MAP::iterator,
           int64, CookiePlatform>(login_cache_->cookie_map_,
                   plat_id, platform);
    if (!r) {
...
void LoginSchdulerManager::SetBatchCookie(const int64 plat_id, const int64 from) {
    std::list<base_logic::LoginCookie> list;
...
bool LoginSchdulerManager::FectchBacthCookies(const int64 plat_id,
        const int64 count, std::list<base_logic::LoginCookie>* list) {
    base_logic::WLockGd lk(lock_);
...
void LoginSchdulerManager::SetCookie(
        const base_logic::LoginCookie& info) {
    CookiePlatform& platform = login_cache_->cookie_map_[info.get_cookie_attr_id()];
...
int64& LoginSchdulerManager::GetDatabaseUpdateTimeByPlatId(const int64 plat_id) {
    return login_cache_->update_time_map_[plat_id];
}
void LoginSchdulerManager::SetUpdateTime(const int64 plat_id, const int64 update_time) {
