...
class THSMainNews(MoreNewsBase):
    ...
    def __analysis(self):
        ...
        info_encode = chardet.detect(self.html_data).get('encoding')  # 通过第3方模块来自动提取网页的编码
        #         #先转换成unicode编码，然后转换系统编码输出
        self.html_data = self.html_data.decode(info_encode, 'ignore').encode(type_encode)
        doc = html.fromstring(self.html_data)
        self.__get_model_url(doc)
        self.__today_focus_news(doc)
    ...
    def __today_focus_news(self, doc):
        """
        今日要闻
        """
        model = '今日要闻'
        picture_new_title = doc.xpath(r'//span[@data-tce-tag="intr"]/text()')[0].encode('ISO-8859-1')
