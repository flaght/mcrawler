...
class YCMarkets(YCAnalysisBase):
    """
    第一财经主页股市分页解析(cid=230&subcid=232)
    """
    ...
    def __get_url_list(self, doc):
        ...
        """
        # 把后面url返回给服务器
        analysis = YCAnalysis('1', '5', doc, 230, 232)
...
def load_data():
    ...
    加载网页html
    """
    if not os.path.exists('./yicai_markets.html'):
        response = urllib2.urlopen('http://www.yicai.com/markets/')
        with open('./yicai_markets.html', 'w') as f:
