# -*- coding:utf-8 -*-
'''
Create on 2015年12月03日

@author: slm
'''
import urllib2
import os
import re
from lxml import html
from schduler.analysis_models.cn_models.cn_analysis_base import CNAnalysisBase
from schduler.analysis_models.analysis_base import AnalysisBase

class CNDetail(CNAnalysisBase):
    '''
    首页详情页(新闻、财经模块)
    '''
    tag = {'rule':'',
           'tag':''}

    def __init__(self, callback=None, **kwargs):
        super(self.__class__, self).__init__(callback, **kwargs)
        self.storage_type = 1
        self._type = '新闻'
        self.current_url = ''
        self.analyzed_info_list = []
        self.__analysis()
        self.analyzed()

    def analyzed(self):
        self.make_storage_cmd_words()
        AnalysisBase.analyzed(self)

    def __analysis(self):
        '''
        解析
        '''
        with open('./21cn_cn_detail.html', 'r') as f:
            self.html_data = f.read()
        doc = html.fromstring(self.html_data)
        self.__get_current_id(doc)
        self.__detail(doc)

    def __get_current_id(self, doc):
        '''
        获得当前current_id
        '''
        self.current_id = re.findall("contentId: (.*?),", self.html_data)[0].encode('utf-8')

    def __detail(self, doc):
        '''
        详情
        current_id:当前新闻id
        date:日期
        content:内容
        comment_num:评论数量
        like_num:点赞数
        unlike_num:踩数量
        comment_detail:评论详情
        '''
        date = doc.xpath(r'//div[@id="article-cmt"]//span[@class="pubTime"]/text()')\
        [0].encode('utf-8')
        content_list = doc.xpath(r'//div[@id="article_text"]//p/text()')
        content = ''
        for info in content_list:
            content += info.strip().encode('ISO-8859-1')
        self.set_analyzed_info_word(self.current_id, date, content, None, None, None, None)

def main():
    '''
    执行
    '''
    analysis = CNDetail()
    print analysis
    for analyzed_info in analysis.analyzed_info_list:
        print analyzed_info.current_id
        print analyzed_info.date
        print analyzed_info.content
        print '================='

def load_data():
    '''
    加载html
    '''
    if not os.path.exists('./21cn_cn_detail.html'):
        response = urllib2.\
        urlopen(r'http://finance.21cn.com/news/cjyw/a/2015/1202/17/30336838.shtml')
        with open('./21cn_cn_detail.html', 'w') as f:
            f.write(response.read())

if __name__ == '__main__':
    load_data()
    main()
