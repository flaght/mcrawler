...
class YCpolitics(YCAnalysisBase):
    '''
    第一财经主页时政分页解析（cid=195&subcid=205）
    '''
    ...
    def __analysis(self):
        ...
        解析(标题，url，类别，行业，模块)
        '''
#         with open('./yicai_politics.html', 'r') as f:
#             self.html_data = f.read()
    ...
    def __get_url_list(self, doc):
        ...
        '''
        #把后面url返回给服务器
        analysis = YCAnalysis('1', '2', doc, 195, 205)
...
def load_data():
    ...
    加载要解析的html
    '''
    if not os.path.exists('./yicai_politics.html'):
        response = urllib2.urlopen('http://www.yicai.com/politics/')
        with open('./yicai_politics.html', 'w') as f:
