...
#                 self.__weibo_microblogger_info(html_info)
    ...
    def __weibo_microblogger_info(self, stock_html_data):
        '''
        微博博主信息（个人简介）
        '''
    ...
    def __get__info(self, weibo_list):
        ...
        for info in weibo_list:
            ...
            #评论、转发、点赞
            bottomn_list = info.xpath(r'div[@class="WB_feed_handle"]//span[@class="pos"]')
            share_num = self.__bottom_list(bottomn_list[1], 1)
            comment_num = self.__bottom_list(bottomn_list[2], 2)
            likes_num = self.__bottom_list(bottomn_list[3], 3)
            self.set_analyzed_info_weibos(self.weibo_id, weibo, share_weibo_id,
                                          share_num, comment_num, likes_num)

    def __share_weibo_id(self, info):
        '''
        博主转发微博的id
        '''
        share_weibos_xpath = \
    ...
    def __get_weibo_detail(self, info):
        '''
        波账户所发微博内容
        '''
        detail_list_xpath = \
    ...
    def __bottom_list(self, bottomn_doc, index):
        '''
        关注、评论、点赞
        '''
        if index == 3:
