...
class THSFinanceFocus(MoreNewsBase):
    '''
    财经要闻（3.财经要闻）
    '''
...
#         with open('./ths_today_list.html', 'r') as f:
...
def load_data():
    ...
    加载网页html
    '''
    if not os.path.exists('./ths_today_list.html'):
        response = urllib2.urlopen('http://news.10jqka.com.cn/today_list/')
        with open('./ths_today_list.html', 'w') as f:
