...
class HbaseManageModel(threading.Thread):
    ...
    def get_row(self, table=None, row_key=None, callback=None):
        """
        get row
        """
        self.task.append({'action': 2,
                          'data': {'table': table, 'rowkey': row_key},
                          'callback': callback})
        return
        if table and row_key:
            try:
                ret = self.hb.table(table).row(row_key)
                return ret
            except Exception, e:
                print 'err:', e
            return None
            try:
                ret = self.client.getRow(table, row_key, None)
                return ret
            except Exception, e:
                print 'error : table:%s rowkey:%s err:%s' % (table, row_key, e)
                return None
        else:
            print 'get row error'
            return None
