...
class IPInfo(object):
    ...
    UA/IP属性
    '''
    __struct_analysis_rule = '=IB31s'
    ip_info_length = struct.calcsize(__struct_analysis_rule)
...
class SockSCrawlerIP(SockHead):
    ...
    def __init__(self, params=None):
        if not params:
            ...
            self.opcode = KidSockOpcode.s_IP_info
            self.type = 1
            # body
        ...
        self.forge_info = []
        while str_body:
            str_task = str_body[0:IPInfo.ip_info_length]
            forge_info = IPInfo(str_task)
            self.forge_info.append(forge_info)
            str_body = str_body[IPInfo.ip_info_length:]
...
def main():
    ...
    package.forge_info = [IPInfo(info.make_package()),
                          IPInfo(info.make_package()),
                          IPInfo(info.make_package()),
                          IPInfo(info.make_package()),
                          IPInfo(info.make_package())]
    package_info = package.make_package()
    # 解包
    sock_crawler_manager_reg = SockSCrawlerIP(package_info)
