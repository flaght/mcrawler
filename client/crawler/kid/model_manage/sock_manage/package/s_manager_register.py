...
class SockSManagerREG(SockHead):
    '''
    爬虫管理器注册成功
    '''
    __struct_analysis_rule = '=I32s'

    def __init__(self, params=None):
        if params == None:
            #head
            super(self.__class__, self).__init__()
            self.opcode = KidSockOpcode.s_manager_register
            self.type = 1
            # body
        ...
        (self.manager_id,
        self.token) = struct.unpack(self.__struct_analysis_rule, str_body)
...
def main():
    ...
    # 解包
    sock_crawler_manager_reg = SockSManagerREG(package_info)
